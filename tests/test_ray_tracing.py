...
# TODO: Add tests for BasicRayTracer and BasicRayTracePath
# (should be mostly the same, but with more tolerance maybe)


@pytest.fixture
def ray_tracer():
    """Fixture for forming basic RayTracer object"""
...
@pytest.fixture
def bad_tracer():
    """Fixture for forming RayTracer object with no solutions"""
...
@pytest.fixture
def out_tracer():
    """Fixture for forming RayTracer object with point above ice"""
    return SpecializedRayTracer(from_point=[100, 200, -500],
                                to_point=[0, 0, 100],
                                ice_model=ArasimIce)
...
class TestSpecializedRayTracer:
    ...
    def test_peak_angle(self, ray_tracer, ray_tracer2, ray_tracer3):
        """Test the angle at which the indirects solutions curve peaks"""
        # TODO: Add peak_angle tests for more from/to_points
        expected = 1.189505626
        assert ray_tracer.peak_angle == pytest.approx(expected)
        expected2 = 1.335645314
        assert ray_tracer2.peak_angle == pytest.approx(expected2)
        expected3 = 1.378722855
        assert ray_tracer3.peak_angle == pytest.approx(expected3)
    ...
    def test_direct_r_max(self, ray_tracer, ray_tracer2, ray_tracer3):
        ...
        assert ray_tracer.direct_r_max == pytest.approx(expected)
        expected2 = 1032.691176
        assert ray_tracer2.direct_r_max == pytest.approx(expected2)
        expected3 = 7472.020234
        assert ray_tracer3.direct_r_max == pytest.approx(expected3)
    ...
    def test_indirect_r_max(self, ray_tracer, ray_tracer2, ray_tracer3):
        ...
        assert ray_tracer.indirect_r_max == pytest.approx(expected)
        expected2 = 1440.711968
        assert ray_tracer2.indirect_r_max == pytest.approx(expected2)
        expected3 = 7453.552318
        assert ray_tracer3.indirect_r_max == pytest.approx(expected3)
    ...
    def test_direct_angle(self, ray_tracer, ray_tracer2, ray_tracer3):
        ...
        assert ray_tracer.direct_angle == pytest.approx(expected)
        assert ray_tracer2.direct_angle is None
        expected3 = 0.108249386
        assert ray_tracer3.direct_angle == pytest.approx(expected3)
    ...
    def test_indirect_angle_1(self, ray_tracer, ray_tracer2, ray_tracer3):
        ...
        assert ray_tracer2.indirect_angle_1 == pytest.approx(expected2)
    ...
    def test_indirect_angle_2(self, ray_tracer, ray_tracer2, ray_tracer3):
        ...
        assert ray_tracer.indirect_angle_2 == pytest.approx(expected)
        expected2 = 1.105377817
        assert ray_tracer2.indirect_angle_2 == pytest.approx(expected2)
        expected3 = 0.081055943
        assert ray_tracer3.indirect_angle_2 == pytest.approx(expected3)
...
class TestSpecializedRayTracePath:
    ...
    def test_beta(self, ray_tracer, ray_tracer2, ray_tracer3):
        ...
        for i, path in enumerate(ray_tracer.solutions+ray_tracer2.solutions
            ...
            assert path.beta == pytest.approx(expected[i])

    def test_z_turn(self, ray_tracer, ray_tracer2, ray_tracer3):
        ...
        for i, path in enumerate(ray_tracer.solutions+ray_tracer2.solutions
                                 +ray_tracer3.solutions):
            assert path.z_turn == pytest.approx(expected[i])
    ...
    def test_emitted_direction(self, ray_tracer, ray_tracer2, ray_tracer3):
        ...
        for i, path in enumerate(ray_tracer.solutions+ray_tracer2.solutions
                                 +ray_tracer3.solutions):
            for j in range(3):
                assert path.emitted_direction[j] == pytest.approx(expected[i][j])
    ...
    def test_received_direction(self, ray_tracer, ray_tracer2, ray_tracer3):
        ...
        for i, path in enumerate(ray_tracer.solutions+ray_tracer2.solutions
                                 +ray_tracer3.solutions):
            for j in range(3):
                assert path.received_direction[j] == pytest.approx(expected[i][j])

    def test_path_length(self, ray_tracer, ray_tracer2, ray_tracer3):
        ...
        for i, path in enumerate(ray_tracer.solutions+ray_tracer2.solutions
                                 +ray_tracer3.solutions):
            assert path.path_length == pytest.approx(expected[i])

    def test_tof(self, ray_tracer, ray_tracer2, ray_tracer3):
        ...
        for i, path in enumerate(ray_tracer.solutions+ray_tracer2.solutions
                                 +ray_tracer3.solutions):
            assert path.tof == pytest.approx(expected[i])
    ...
    def test_attenuation(self, frequency, ray_tracer, ray_tracer2, ray_tracer3):
        ...
        for i, path in enumerate(ray_tracer.solutions+ray_tracer2.solutions
            ...
            assert (path.attenuation(frequency)[0] ==
                    pytest.approx(expected[frequency][i]))

...
class AntennaSystem:
    ...
    @property
    def waveforms(self):
        """The antenna system signal + noise at each triggered hit."""
...
class Detector:
    ...
    antenna_positions : list
        List of the positions of the antennas generated by the
        ``set_positions`` method.
    subsets : list
        List (potentially with sub-lists) of the antenna objects which make up
        the detector.
    ...
    are detectors, and so on. For example, a string of antennas could be set up
    as a subclass of ``Detector`` which sets up some antennas in a vertical
    line. Then a station could be set up as a subclass of ``Detector`` which
    sets up multiple instances of the string class at different positions. Then
    a final overarching detector class can subclass ``Detector`` and set up
    mutliple instances of the station class at different positions. In this
    example the ``subsets`` of the overarching detector class would be the
    station objects, the ``subsets`` of the station objects would be the string
    objects, and the ``subsets`` of the string objects would finally be the
    antenna objects. But the way the iteration of the ``Detector`` class is
    built, iterating over that overarching detector class would iterate
    directly over each antenna in each string in each station as a simple 1D
    list.
    ...
    def build_antennas(self, *args, **kwargs):
        ...
        subset ``build_antennas`` methods are automatically passed to.

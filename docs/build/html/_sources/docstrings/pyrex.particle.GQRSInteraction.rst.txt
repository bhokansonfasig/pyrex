pyrex.particle.GQRSInteraction
==============================

.. currentmodule:: pyrex.particle

.. autoclass:: GQRSInteraction

   

   .. HACK -- the point here is that we don't want this to appear in the output, but the autosummary should still generate the pages.
      .. autosummary::
         :toctree:
      
         GQRSInteraction.choose_inelasticity
         GQRSInteraction.choose_interaction
         GQRSInteraction.choose_shower_fractions



   

   .. HACK -- the point here is that we don't want this to appear in the output, but the autosummary should still generate the pages.
      .. autosummary::
         :toctree:
      
         GQRSInteraction.cross_section
         GQRSInteraction.interaction_length
         GQRSInteraction.kind
         GQRSInteraction.total_cross_section
         GQRSInteraction.total_interaction_length

